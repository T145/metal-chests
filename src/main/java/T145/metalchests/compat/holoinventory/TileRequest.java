...
public class TileRequest extends RequestMessage {
	...
	public static class Handler implements IMessageHandler<TileRequest, ResponseMessage> {
		...
		public ResponseMessage onMessage(TileRequest message, MessageContext ctx) {
			...
			if (te == null || Helper.banned.contains(te.getClass().getCanonicalName()) || (te instanceof ILockableContainer && !ctx.getServerHandler().player.canOpen(((ILockableContainer) te).getLockCode()))) {
				return null;
			}
			...
			if (te instanceof TileEntityEnderChest) {
				...
				if (b instanceof BlockChest) {
					...
					if (i != null) {
						return new PlainInventory(message.pos, i);
					}
				...
				if (te instanceof TileMetalChest) {
					...
					return new PlainInventory(message.pos, "tile.metalchests:metal_chest." + chest.getType().getName() + ".name", iih);
				}
