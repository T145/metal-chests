...
import T145.metalchests.tiles.TileMetalTank.InternalTank;
...
public class RenderMetalTank extends TileEntitySpecialRenderer<TileMetalTank> {
	...
	private void renderFluid(TileMetalTank tank) {
		if (tank.hasWorld() && tank.hasFluid()) {
			GlStateManager.pushMatrix();
			GlStateManager.enableBlend();
			InternalTank internalTank = tank.getTank();
			renderFluidCuboid(tank, 0.15D, 0.0D, 0.15D, 0.0D, 0.0D, 0.0D, 0.7D, internalTank.getFluidAmount() / internalTank.getCapacity(), 0.7D);
	...
	public static void renderFluidCuboid(TileMetalTank te, double x, double y, double z, double x1, double y1, double z1, double x2, double y2, double z2) {
		FluidStack fluid = te.getTank().getFluid();
		int color = fluid.getFluid().getColor(fluid);
		renderFluidCuboid(te, fluid, te.getPos(), x, y, z, x1, y1, z1, x2, y2, z2, color);
	}
	 ...
	 */
	public static void renderFluidCuboid(TileMetalTank te, FluidStack fluid, BlockPos pos, double x, double y, double z, double x1, double y1, double z1, double x2, double y2, double z2, int color) {
		...
		final InternalTank liquid = te.getTank();
		...
		if (/*liquid.getFluidAmount() != Fluid.BUCKET_VOLUME * ConfigLoader.maxTankCapacity / 1000 ||*/ (tankAbove != null
				&& tankAbove instanceof TileMetalTank
				&& (((TileMetalTank) tankAbove).getTank().getFluidAmount() <= 0
				|| (((TileMetalTank) tankAbove).getTank().getFluid() != null && ((TileMetalTank) tankAbove).getTank()
				.getFluid().getFluid() != te.getTank().getFluid().getFluid())))) {
			putTexturedQuad(renderer, still, x1, y1, z1, x2 - x1, y2 - y1, z2 - z1, EnumFacing.UP, color, brightness,
					false);
		}

		if (tankBelow != null && tankBelow instanceof TileMetalTank
				&& ((TileMetalTank) tankBelow).getTank().getFluid() != null
				&& ((TileMetalTank) tankBelow).getTank().getFluid().getFluid() != te.getTank().getFluid().getFluid()) {
	...
	public static void renderStackedFluidCuboid(FluidStack fluid, double px, double py, double pz, BlockPos pos, BlockPos from, BlockPos to, double ymin, double ymax, float offsetToBlockEdge) {
		...
		// ymin's decimal is bigger than ymax's decimal (causing UV over 1)
		...
		// face array becoming 2.3, 3, 4, 5.1
		...
		// 2, 3, 3 as we then have middle ints
		if (ymax % 1d == 0)
			yd--;
	...
	/**
	 * Similar to putTexturedQuad, except its only for upwards quads and a rotation
	 * is specified
	 */
	...
	public static void putRotatedQuad(BufferBuilder renderer, TextureAtlasSprite sprite, double x, double y, double z, double w, double d, EnumFacing rotation, int r, int g, int b, int a, int light1, int light2, boolean flowing) {
		...
		// when rotating by 90 or 270 the dimensions switch, so switch the U and V
		// before hand
		...
		// we want to start from the bottom for north or west textures as otherwise UV
		// is backwards
		// we also want to start from the bottom for flowing fluids, and both should
		// cancel
