...
public class ItemMinecartMetalChest extends ItemBase {
	...
	private static final IBehaviorDispenseItem MINECART_DISPENSER_BEHAVIOR = new BehaviorDefaultDispenseItem() {
		...
		public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
			...
			EntityMinecartMetalChestBase entityminecart = EntityMinecartMetalChestBase.create(world, d0, d1 + d3, d2, MetalChestType.byMetadata(stack.getItemDamage()));
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!BlockRailBase.isRailBlock(iblockstate)) {
			...
			if (!worldIn.isRemote) {
				...
				EntityMinecartMetalChestBase cart = EntityMinecartMetalChestBase.create(worldIn, (double) pos.getX() + 0.5D, (double) pos.getY() + 0.0625D + d0, (double) pos.getZ() + 0.5D, MetalChestType.byMetadata(itemstack.getItemDamage()));
	...
	public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> items) {
		if (tab == MetalChests.TAB) {
			for (int meta = 0; meta < getItemTypes().length; ++meta) {
				MetalChestType type = MetalChestType.byMetadata(meta);
